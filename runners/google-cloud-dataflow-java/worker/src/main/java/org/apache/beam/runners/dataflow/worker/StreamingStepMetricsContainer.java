...
public class StreamingStepMetricsContainer implements MetricsContainer {
  ...
  private MetricsMap<LockFreeHistogram.HistogramKey, LockFreeHistogram>
  ...
  private final ConcurrentHashMap<MetricName, LabeledMetricNameUtils.ParsedMetricName> parsedPerWorkerMetricsCache;
  ...
  private StreamingStepMetricsContainer(
      ...
      ConcurrentHashMap<MetricName, LabeledMetricNameUtils.ParsedMetricName> parsedPerWorkerMetricsCache,
      Clock clock) {
    this.stepName = stepName;
  ...
  static StreamingStepMetricsContainer forTesting(
    ...
    return new StreamingStepMetricsContainer(
        stepName, perWorkerCountersByFirstStaleTime, perWorkerCounters, parsedPerWorkerMetricsCache, clock);
  }
  ...
  private void deleteStaleCounters(
    ...
    // Remove potentially deleted metric names from the cache. If these metrics are non-zero valued in the future,
  ...
  Iterable<PerStepNamespaceMetrics> extractPerWorkerMetricUpdates() {
    ...
    return MetricsToPerStepNamespaceMetricsConverter.convert(stepName, counters, histograms, parsedPerWorkerMetricsCache);
  }
